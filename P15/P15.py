...
# Configura la comunicación con Arduino (ajusta el puerto COM según tu configuración)
ser = serial.Serial('COM3', 9600)

def PedirDistancia():
    #Lee la línea enviada por Arduino
    linea = ser.readline().decode().strip()
    distancia_str = linea.split(",")
    distancia_cms = float(distancia_str[0])
    indicatorL_label.config(text="Nivel: "+str(distancia_cms))
    return distancia_cms
